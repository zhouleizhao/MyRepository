...
- (void)getData{
    [App_ZLZ_Helper sendDataToServerUseUrl:@"user/evaluate/page" dataDict:@{@"orderNum":self.orderNum} type:RequestType_Post loadingTitle:@"" sucessTitle:@"" sucessBlock:^(NSDictionary *resObj) {
        [self.driverImageView sd_setImageWithURLNew:[NSURL URLWithString:resObj[@"data"][@"avatar"]] placeholderImage:[UIImage imageNamed:@"logo"]];
...
- (IBAction)sendClick:(id)sender {
    ...
    [App_ZLZ_Helper sendDataToServerUseUrl:@"user/order/comment" dataDict:param type:RequestType_Post loadingTitle:@"正在提交..." sucessTitle:@"评论成功" sucessBlock:^(NSDictionary *) {
        [self.navigationController popToViewController:self.navigationController.viewControllers[self.navigationController.viewControllers.count - 3] animated:YES];
    } failedBlock:^(NSError *) {
